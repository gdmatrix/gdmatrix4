...
import org.gdmatrix.modgen.schema.TypifiedComponent;
...
{
  public HTMLModuleGenerator(ModuleFactory moduleFactory)
  {
    super(moduleFactory);
  }
  ...
  {
    try (PrintWriter printer = getWriter(module.getName() + ".html"))
    {
      ...
      printer.println("<td class=\"col2\"><span class=\"code\">" +
        module.getManager() + "Service</span></td>");
      printer.println("</tr>");

      printer.println("<tr>");
      printer.println("<td class=\"col1\">Port:</td>");
      printer.println("<td class=\"col2\"><span class=\"code\">" +
        module.getManager() + "Port</span></td>");
      printer.println("</tr>");
      ...
      String wsdlLocation = module.getAttribute("jaxws.wsdlLocation");
      if (wsdlLocation != null)
      {
        printer.println("<tr>");
        printer.println("<td class=\"col1\">WSDL:</td>");
        printer.println("<td class=\"col2\"><a href=\"" +
          wsdlLocation + "\"><span class=\"code\">" +
      ...
      for (Import _import : module.getImports())
      {
        ...
        if (moduleName != null) // module import
        {
          ...
          if (importModule == null)
            throw new RuntimeException("Invalid module " + moduleName);
  ...
  private void updateModulesIndex(String name, String fragment) throws Exception
  {
    ...
    try (PrintWriter printer = new PrintWriter(file, "UTF-8"))
    {
  ...
  {
    ArrayList<Type> entities = new ArrayList<>();
    ArrayList<Type> structs = new ArrayList<>();
    ArrayList<Type> enumerations = new ArrayList<>();
    for (Type type : module.getTypes())
    {
  ...
  private void generateComplexType(PrintWriter printer, ComplexType complexType)
  {
    ...
    List<NamedComponent> list = new ArrayList<>();
    ...
    List<Property> attributes = complexType.getProperties().stream()
      .filter(property -> "true".equals(property.getAttribute("jaxws.isAttribute")))
      .collect(Collectors.toList());
    generateAttributes(printer, attributes);
  ...
  private void generateAttributes(PrintWriter printer, List<Property> attributes)
  {
  ...
  private void generateAttribute(PrintWriter printer, Property attribute)
  {
    boolean required = !"0".equals(attribute.getMinOccurs());
    printer.println("<tr>");
    printer.println("<td><span class=\"code\">" + attribute.getName() + "</span></td>");
    printer.println("<td><span class=\"code\">" + attribute.getType() + "</span></td>");
    printer.println("<td><span class=\"code\">" + required + "</span></td>");
    printer.println("</tr>");
  }
  ...
  private String getTypeReference(String qname)
  {
    ...
    if (moduleFactory.getModule(moduleName) == null)
    {
      reference = qname;
    }
    else // type from module
    {
      ...
      reference = "<a href=\"" + moduleName + ".html#type-" + name + "\">" +
        qname + "</a>";
    }
    return reference;
  }
    ...
    Operation operation)
  {
    ...
    for (Parameter parameter : operation.getParameters())
    {
      ...
      else
      {
        ...
        if (type instanceof ComplexType)
        {
          generateSampleAttributes(printer, ((ComplexType) type).getProperties());
    ...
    Operation operation)
  {
    ...
    if (response != null)
    {
      ...
      else
      {
        ...
        if (type instanceof Entity)
        {
          generateSampleAttributes(printer, ((Entity)type).getProperties());
          printer.println("<span class=\"tag\">&gt;</span>");
          generateSampleInstance(printer, (Entity)type, 8);
        }
        else if (type instanceof Struct)
        {
          generateSampleAttributes(printer, ((Struct)type).getProperties());
          printer.println("<span class=\"tag\">&gt;</span>");
  ...
  {
    List<Property> attributes = properties.stream()
      .filter(property -> "true".equals(property.getAttribute("jaxws.isAttribute")))
      .collect(Collectors.toList());
  ...
  public static void main(String[] args) throws Exception
  {
    if (args.length < 2)
    {
      System.out.println("Usage: java " +
        HTMLModuleGenerator.class.getCanonicalName() +
        " <module_dir> <output_dir> <css_uri>");
    }
    else
    {
      ModuleFactory moduleFactory = new ModuleFactory(args[0]);
      HTMLModuleGenerator gen = new HTMLModuleGenerator(moduleFactory);
      gen.setOutputDirectory(new File(args[1]));
      if (args.length >= 3)
      {
        gen.setCssUri(args[2]);
      }
